...
<div>
    <Navigate to="invoice/{invoice.id}" styles="flex max-lg:flex-col cursor-pointer gap-4 mb-4 dark:text-white text-gray-600 rounded-3xl no-underline py-7 px-8 bg-mainBg dark:bg-regColor items-center">
        <div class="flex items-center basis-3/5 gap-4 max-lg:flex-col max-lg:gap-3">
            <span class="flex-1 uppercase text-sm">
                #{invoice.id.slice(18, 24)}
            </span>
            <span class="flex-1 text-sm max-lg:mt-4">
        ...
        <div class="flex gap-4 basis-2/5 items-center max-lg:flex-col">
